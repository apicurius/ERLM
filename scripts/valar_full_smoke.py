...
gold = (
    str(ast.literal_eval(meta["answer"])[0]) if meta["answer"].startswith("[") else meta["answer"]
)
check(
    f"score(gold={gold!r})==1.0",
    run(_score_oolong(info, {"rlm_final_answer": f"Answer: {gold}"})) == 1.0,
)
...
check(
    "score(Exact Answer: gold)==1.0",
    run(_score_browsecomp_plus(info, {"rlm_final_answer": f"Exact Answer: {g}"})) == 1.0,
)
...
check(
    f"score(gold letter={letter})==1.0",
    run(_score_longbench_codeqa(info, {"rlm_final_answer": letter})) == 1.0,
)
check(
    "score(wrong letter)==0.0", run(_score_longbench_codeqa(info, {"rlm_final_answer": "Z"})) == 0.0
)
...
shaped = rlm_train.EfficiencyGatedRubric(
    correctness=c1,
    min_iterations=2,
    shaping_coef=0.2,
    max_iterations=20,
...
wrong = rlm_train.EfficiencyGatedRubric(
    correctness=c0,
    min_iterations=2,
    shaping_coef=0.2,
    max_iterations=20,
...
cheap = {
    "rlm_iterations": 3,
    "rlm_sub_llm_calls": 3,
    "rlm_sub_llm_tokens": 35000,
    "rlm_final_answer": "x",
}
pricey = {
    "rlm_iterations": 18,
    "rlm_sub_llm_calls": 60,
    "rlm_sub_llm_tokens": 190000,
    "rlm_final_answer": "x",
}
...
print(
    f"  control(l=0)={r_stock:.4f}  treat-cheap(l=0.2)={r_cheap:.4f}  treat-pricey={r_pricey:.4f}  wrong={r_wrong:.4f}"
)
